...
float SHT1x::readTemperatureC()
{
  ...
  const float DA1 = -40.0;  // for 14 Bit @ 5V
  const float DA2 =   0.01; // for 14 Bit DEGC

  // Fetch raw value
  _val = readTemperatureRaw();

  // Convert raw value to degrees Celsius
  _temperature = (_val * DA2) + DA1;
...
float SHT1x::readTemperatureF()
{
  ...
  // Conversion coefficients from SHT15 datasheet
  const float DA1 = -40.0;   // for 14 Bit @ 5V
  const float DA2 =   0.018; // for 14 Bit DEGF
...
float SHT1x::readHumidity()
{
  ...
  // Conversion coefficients from SHT15 datasheet
  const float CA1 = -4.0;       // for 12 Bit
  const float CA2 =  0.0405;    // for 12 Bit
  const float CA3 = -0.0000028; // for 12 Bit
  ...
  _linearHumidity = CA1 + CA2 * _val + CA3 * _val * _val;
